...
constexpr std::size_t DIVERGENCE_ITERATIONS = 10;
...
public:
    ...
    float vx[AIR_YRES + 1][AIR_XRES + 1];
    float vy[AIR_YRES + 1][AIR_XRES + 1];
    float new_vx[AIR_YRES + 1][AIR_XRES + 1];
    float new_vy[AIR_YRES + 1][AIR_XRES + 1];
...
private:
    ...
    void fill_edges();
    float sample_field_bilinear(float x, float y, float field[AIR_YRES + 1][AIR_XRES + 1], float dx, float dy);
